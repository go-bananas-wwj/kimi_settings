...
def target_paths(target: str) -> dict[str, Path]:
    ...
    paths = {
        ...
        "claude_skills": Path(os.environ.get("PAPERSPINE_CLAUDE_SKILLS_DIR", home / ".kimi" / "skills")),
        "claude_commands": Path(os.environ.get("PAPERSPINE_CLAUDE_COMMANDS_DIR", home / ".kimi" / "commands")),
...
def run(args: argparse.Namespace) -> int:
    ...
    if comparison >= 0:
        ...
        if comparison == 0 and not args.check_only:
            ...
            sync_skill_overrides(Path.home() / ".kimi")
        return 0
